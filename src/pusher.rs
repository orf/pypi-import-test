...
use git2::{Commit, Error, Odb, Oid, Repository, TreeWalkMode};
...
pub fn push(repo: PathBuf) {
    ...
    let mut current_run = CurrentRun { name: "".to_string(), items: vec![] };
    ...
    odb.foreach(|v| {
        match repo.find_commit(*v) {
            Ok(c) => {
                let package_name = c.message().unwrap().trim().splitn(2, ' ').next().unwrap();

                if package_name != current_run.name {
                    let new_current_run = CurrentRun { name: package_name.to_string(), items: vec![] };
                    let old_current_run = std::mem::replace(&mut current_run, new_current_run);
                    if !old_current_run.items.is_empty() {
                        serde_json::to_writer(&mut stdout, &ordered_commits(old_current_run, &repo)).unwrap();
                        stdout.write_all("\n".as_ref()).unwrap();
                    }
                }
...
lazy_static! {
    static ref INFO_REGEX: Regex = Regex::new("[^\\s]* (?P<version>[^\\s]*) \\((?P<filename>[^\\s]*)\\)").unwrap();
}
...
fn ordered_commits(mut run: CurrentRun, repo: &Repository) -> PushStrategy {
    ...
    for commit in run.items {
        ...
        sub_tree.walk(TreeWalkMode::PostOrder, |_, _| {
            running_total += 1;
            total_files += 1;
            0
        }).unwrap();
