...
pub fn merge_all_branches(into: PathBuf, mut repos: Vec<PathBuf>) -> anyhow::Result<()> {
    ...
    std::process::Command::new("git").current_dir(target_repo.path()).args(&["repack", "--max-pack-size=1500m", "-k", "-a", "-d", "--window=5", "--depth=20", "--write-bitmap-index", "--threads=1"]).status().unwrap();
